...
function setup(){
  ...
  run1 = createSprite(500,600,20,50);
  run1.addAnimation("running", run1I);
  run1.scale = 0.7;

  bgGr = createSprite(500,750,1000000,15);
...
function draw(){
  ...
  image(bg2,0,0,displayWidth-30,displayHeight-150);
  ...
  if(gameState === PLAY){
    ...
    if(keyIsDown(UP_ARROW) && run1.y >= 595.5) {
      run1.velocityY = -29;
    }
  ...
  if (gameState === END) {
    ...
    var GO;
    GO = createSprite(displayWidth/2,displayHeight/2,displayWidth-30,displayHeight-150);
...
function Obstacles() {
  if(frameCount % 150 === 0) {
    var obstacle = createSprite(1500,650,100,100);
